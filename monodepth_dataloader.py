...
class MonodepthDataloader(object):
    ...
    def __init__(self, data_path, filenames_file, params, dataset, mode):
        ...
        if mode == 'test' and not self.params.do_stereo:
            ...
            left_image_path  = tf.string_join([self.data_path, split_line[0]])
            label_image_path = tf.string_join([self.data_path, split_line[1]])
            left_image_o  = self.read_image(left_image_path)
            label_image_o = self.read_image_label(label_image_path)
        else:
            left_image_path  = tf.string_join([self.data_path, split_line[0]])
        ...
        if mode == 'segment' :
            ...
            #do_flip = tf.random_uniform([], 0, 1)
            #left_image  = tf.cond(do_flip > 0.5, lambda: tf.image.flip_left_right(right_image_o), lambda: left_image_o)
            #label_image = tf.cond(do_flip > 0.5, lambda: tf.image.flip_left_right(left_image_o),  lambda: right_image_o)
            left_image = left_image_o
            label_image = label_image_o
            # randomly augment images
            do_augment  = tf.random_uniform([], 0, 1)
            left_image = tf.cond(do_augment > 0.5, lambda: self.augment_image(left_image), lambda: (left_image))

            left_image.set_shape( [None, None, 3])
            label_image.set_shape([None, None, 1])

            # capacity = min_after_dequeue + (num_threads + a small safety margin) * batch_size
            min_after_dequeue = 2048
            capacity = min_after_dequeue + 4 * params.batch_size
            self.left_image_batch, self.label_image_batch = tf.train.shuffle_batch([left_image, label_image],
                        params.batch_size, capacity, min_after_dequeue, params.num_threads)
    ...
    def augment_image(self, left_image):
        # randomly shift gamma
        random_gamma = tf.random_uniform([], 0.8, 1.2)
        left_image_aug  = left_image  ** random_gamma
